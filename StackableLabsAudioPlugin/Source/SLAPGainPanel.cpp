	...
	: SLAPPanelBase(inProcessor)
{
	...
	_vuMeter = std::make_unique<SLAPVuMeter>(inProcessor);
	_vuMeter->setBounds((getWidth()* 0.5) - (meter_width * 0.5), 
						getHeight() * 0.55 - (meter_width * 0.5), 
						meter_width, 
						getHeight() * 0.45);
	addAndMakeVisible(_vuMeter.get());
}
...
{
	_slider = std::make_unique<SLAPParameterSlider>(_processor->parameters, SLAPParameterId[inParameterId], SLAPParameterLabel[inParameterId]);
...
void SLAPGainPanel::paint(Graphics& g)
{
	...
	if(_slider)
	{
		paintComponentLabel(g, _slider.get());
	}
}
